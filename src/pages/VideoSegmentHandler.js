...
const handleVideoDrop = async (e, draggingItem, dragLayer, mouseX, mouseY, timeScale, dragOffset, snapIndicators) => {
  ...
  if (!draggingItem) {
    ...
    if (dataString) {
      ...
      if (data.type === 'media') {
        ...
        const token = localStorage.getItem('token');
        const filePath = await getAccessibleUrl(video.cdnUrl, video.presignedUrl, token); // Use CDN or presigned URL
        ...
        newVideoLayers[targetLayer].push({
          ...
          filePath, // Use resolved URL
          cdnUrl: video.cdnUrl, // Store cdnUrl
          presignedUrl: video.presignedUrl, // Store presignedUrl
  ...
  const token = localStorage.getItem('token');
  const filePath = await getAccessibleUrl(draggingItem.cdnUrl, draggingItem.presignedUrl, token); // Use CDN or presigned URL
...
const getAccessibleUrl = async (cdnUrl, presignedUrl, token) => {
  try {
    const response = await axios.get(
      `${API_BASE_URL}/projects/check-cdn-availability`,
      {
        params: { cdnUrl },
        headers: { Authorization: `Bearer ${token}` },
      }
    ...
    console.warn(`CDN URL unavailable: ${cdnUrl}. Falling back to presigned URL.`);
    return presignedUrl;
  } catch (error) {
    console.error('Error checking CDN availability:', error.response?.data || error.message);
    return presignedUrl;
  }
};
...
const handleVideoSplit = async (item, clickTime, layerIndex) => {
  ...
  try {
    ...
    const firstPartVideoDuration = firstPartDuration * speed;
    const originalVideoStartTime = roundToThreeDecimals(item.startTimeWithinVideo || 0);
    const firstPartKeyframes = item.keyframes || {};

    const filePath = await getAccessibleUrl(item.cdnUrl, item.presignedUrl, token); // Use CDN or presigned URL
    ...
    setVideoLayers((prev) => {
      ...
      updatedLayer[secondPartIndex] = {
        ...
        filePath, // Use resolved URL
        cdnUrl: item.cdnUrl, // Store cdnUrl
        presignedUrl: item.presignedUrl, // Store presignedUrl
        positionX: secondPart.positionX ?? 0,
        positionY: secondPart.positionY ?? 0,
        scale: secondPart.scale ?? 1,
        rotation: secondPart.rotation ?? 0,
